...
The HPF model is more powerful than basic Poisson
factorization. Hierarchical models generally allow for the sharing of
statistical strength. In the HPF, for example, learning about the
number of items consumed by one set of users provides weak, indirect
evidence relevant to other users consumption. This can improve
predictions for existing users and new users. A similar argument
favors hierarchy over item popularities. In the HPF, we fix the shape
parameters and share them across users and items. For a large shape,
coefficients will tend to have similar magnitude, and the
representations will be dense. In contrast, a small shape will result
in a sparse representation.  Finally, the choice of the \gam~prior is
motivated by conjugacy to the \gam~distributions with fixed shape that
govern the weights. This results in a computationally efficient
algorithm.
