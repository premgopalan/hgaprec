...
When applied to explicit data, Gaussian models are fit only to the
observed ratings~\cite{Koren:2009}, inferring distributions over user
preference. For each user, the items she did not consumed, i.e., the
zero observations, are treated as ``missing data''. Gaussian models
make up the state of the art in this setting.
...
like~\cite{Hu:2008p9402}. In this setting, the models based on
Gaussian likelihoods (i.e., squared loss), give equal weight to
consumed and unconsumed items.  Consequently, when faced with a sparse
matrix and implicit feedback, matrix factorization places more total
emphasis on the unconsumed user/item pairs.
