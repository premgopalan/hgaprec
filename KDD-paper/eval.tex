...
\caption{Predictive performance on data sets. The top and bottom plots
  show the mean precision and mean recall at 20 recommendations,
  respectively. While baseline performance varies across data sets,
  HPF and BPF consistently outperform competing methods.}
...
sampling from missing ratings in the training set~\cite{Dror:2012a}.
...
generate negative examples.\footnote{We also compared this to a
  ...
  popularity-based sampling performed better.} Although the explicit
feedback setting does not strictly require subsampled negative
examples,
...
\item {\bf PMF (MCMC)} The fully Bayesian treatment of the
  ...
  the HPF variational inference algorithm converges within 13 hours on
  the Netflix data.
...
%% jake: i merged this para into above; note: i removed the point
%% about ``dozens of iterations''
...
\caption{Predictive performance across users. The top and bottom plots
  show mean precision and mean recall at 20 recommendations,
...
changes in the hyper-parameters. To enforce sparsity, we set the shape
hyperparameters $a'$, $a$, $c$ and $c'$ to provide an exponentially
shaped prior \gam~distribution. We fixed each hyperparameter at
$0.3$. We set the scale parameters $b'$ and $d'$ to 1. We used the
same settings across all data sets.
...
method. For each user, we compute precision-at-$M$, which
measures the fraction of relevant items in the user's top-$M$
recommendations. Likewise, we compute recall-at-$M$, which captures
the fraction of items in the test set present in the top $M$
recommendations.

\myfig{precision_recall} shows the mean precision and mean recall at
20 recommendations for each method and data sets. We see that HPF and
BPF outperform other methods on all data sets by a sizeable
margin. Poisson factorization provides high-quality
recommendations---a relatively high fraction of items recommended by
HPF and BPF are found to be relevant, and many relevant items are
...
performance for users of varying activity, measured by percentile. For
example, the 10\% mark shows mean performance across the bottom 10\%
of users, who are least active; the 90\% mark shows the mean
performance for all but the top 10\% of most active users. Here we see
that Poisson factorization outperforms other methods for users of all
activity levels---both the ``light'' users who constitute the
majority, and the relatively few ``heavy'' users who consume
more---for all data sets. Furthermore, we see that higher levels of
user activity enable us to better estimate user preferences and
improve the quality of recommendations, as measured by mean
precision. We note that the decrease in mean recall as we consider
...
active counterparts. Regardless, Poisson factorization returns a
larger set of relevant recommendations to users compared to other
approaches.
...
precision and mean recall. Knowing how many articles a user has read
in one component helps predict her consumption on other
