...
%% assignment of per-observation (class) confidence weights may require
%% exhaustive search via cross-validation, or other model selection
%% methods, hindering scalability.
...
An issue that we found in Gaussian MF with subsampled zeros, fit using
SGD, is that it systematically overestimates the users' budgets. We
confirmed this in \mysec{eval} using a posterior predictive
...
%As shown in \mysec{model}, Poisson factorization (PF) captures real consumption
%data, specifically that users have finite (and varied) resources with
%which to consume items.
%To see this, we can rewrite the model as a
...
%he model accurately captures the distribution of budgets then
%consumed items carry more weight than unconsumed items, because
%unconsumed items can be partially explained by a lack of resources.
%We conjecture that 
...
%\mysec{model}.
%We discuss additional related recommendation methods in \mysec{eval}.
