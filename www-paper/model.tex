...
In so-called ``implicit'' consumer data that we consider here,
$y_{ui}$ equals one if user $u$ consumed item $i$ and zero otherwise.
User behavior data, such as purchases, clicks, or views, are typically
...
In fact, this is encoded in its assumptions. We can rewrite the
Poisson observation model as a two stage process where a user $u$
...
effectively the same as learning user budgets and how they choose to
spend their budgets.
