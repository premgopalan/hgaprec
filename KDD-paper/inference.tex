...
\label{sec:inference}
In the above section we discussed the process for and properties of
generating ratings from the HPF model assuming known model
parameters. In practice, however, we are faced with the opposite
scenario: we are given a set of observed ratings and would like to
infer parameters that explain these ratings, for the purpose of
recommending new content to users. In principle this amounts to a
simple application of Bayes' rule to compute the posterior over the
latent factors given the observed ratings. In this section we discuss
the details and practical challenges of posterior inference for HPF,
and present a mean-field variational inference algorithm as a
practical and scalable alternative that accommodates data sets with
...
\paragraph{Mean-field variational inference} The latent variables in
...
\paragraph{Variational algorithm}
