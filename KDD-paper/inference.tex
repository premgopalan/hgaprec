  ...
  \begin{framed}
    ...
    \item For each user, update the user weight and activity parameters:
      \begin{align}
        ...
        \kappa_{u}^\rate & = \frac{a'}{b'} + \sum_k \Psi(\gamma_u^{\shape}) - \log(\gamma_u^{\rate})\nonumber
      \end{align}
    \item For each item, update the item weight and popularity parameters:
      \begin{align}
        ...
        \tau_{i}^\rate & = \frac{c'}{d'} + \sum_k \Psi(\lambda_i^{\shape}) -
        \log(\lambda_i^{\rate})\nonumber
      \end{align}
    \end{enumerate}
\end{framed}
...
superscript ``rte''.  (We give a detailed derivation in the Appendix.)
