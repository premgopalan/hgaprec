...
public:
  ...
  Rating(): inherited() { }
  Rating(uint32_t a, uint32_t b): inherited(a,b) { }
};
...
public:
  ...
  double min(uint32_t &idx) const;
  double minpositive(uint32_t &idx) const;
  double norm() const;

  void set_elements(T v);
  void zero();
  void copy_from(const D1Array<T> &a);
  void copy_from(const T *x);
...
typedef D1Array<unsigned char> bArray;
...
D1Array<T>::sum(uint32_t maxn) const
{
  if (maxn == 0)
    maxn = _n;
  T s = .0;
...
template<class T> inline double
D1Array<T>::min(uint32_t &idx) const
{
  double minv = INT_MAX;
  for (uint32_t i = 0; i < n(); ++i) {
    if (_data[i] < minv) {
...
template<class T> inline double
D1Array<T>::minpositive(uint32_t &idx) const
{
  double minv = INT_MAX;
  for (uint32_t i = 0; i < n(); ++i) {
    if (_data[i] > 0 && _data[i] < minv) {
      minv = _data[i];
      idx = i;
    }
  }
...
D1Array<double>::s(uint32_t maxn) const
{
  ostringstream sa;
...
template<> inline string
D1Array<uint32_t>::s(uint32_t maxn) const
{
  ostringstream sa;
  uint32_t n = _n > ROW_LIMIT ? ROW_LIMIT : _n;
  if (maxn && n > maxn)
...
template<> inline string
D1Array<KV>::s(uint32_t maxn) const
{
  ostringstream sa;
  uint32_t n = _n > ROW_LIMIT ? ROW_LIMIT : _n;
  if (maxn && n > maxn)
...
public:
  ...
  void slice_conv(uint32_t dim, uint32_t p, D1Array<double> &v) const;
...
//
// slice an byte matrix into a uint32_t row/col
// useful for slicing adjacency matrices
//

template<class T> inline void
D2Array<T>::slice_conv(uint32_t const_dim, uint32_t p, D1Array<double> &v) const
{
  assert (const_dim < 2);
  if (const_dim == 0) {
    assert (p < _m && v.n() == _n);
